...
class FindTwin:
    ...
    def get_tt_times(self, input_dict, filename, seis, logfile, outfile, fail):
        ...
        ** param == return **
        ...
        if fail:
            ...
            self.write_to_dict(input_dict,['tlim','ttt'],[tlim,ttt])
    ...
    def primary_qc(self, input_dict, filename, seis, logfile, outfile, fail):
        ...
        ** param == return **
        ...
        if fail:
            ...
            tlim = self.read_from_dict(input_dict,['tlim'])[0]
            ...
            if t[0] > tlim[0] or t[-1] - tlim[0] < min_sig_win:
                ...
                ###
                self.tk.print_log(params_in, logfile, f'{log_statement:s}  ,  **too short [t = {t[0]:.1f}-{t[-1]:.1f}s; sigwin = {tlim[0]:.1f}-{tlim[1]:.1f}s -FAILED- ]')
                ###
            ...
            if len(id_noise)*dt <=  min_nois_win: # use last quarter for snr
                ...
                ###
                self.tk.print_log(params_in, logfile, f'{log_statement:s}  ,  **use last quarter ({t[id_noise][0]:.1f}-{t[id_noise][-1]:.1f}s) as noiswin [t = {t[0]:.1f}-{t[-1]:.1f}s, first arrival = {t[0]:.1f}s]')
                ###
            ...
            seis.filter('bandpass',freqmin = 1/params_in['T2'],freqmax = 1/params_in['T1'],corners = 4,zerophase = True)


            if len(id_sig) ==  0 or len(id_noise) ==  0:
                fail = 1
                ###
                self.tk.print_log(params_in, logfile, f'{log_statement:s}  ,  **poor sig/noise win selection: skip -FAILED- ]')
                ###
                return input_dict, filename, seis, logfile, outfile, fail
            ...
            if nois_pow ==  0 or nois_pow ==  np.nan:
                fail = 1
                ###
                self.tk.print_log(params_in, logfile, f'{log_statement:s}  ,  **poor noise variance: skip -FAILED- ]')
                ###
                return input_dict, filename, seis, logfile, outfile, fail
            ...
            ###
            self.tk.print_log(params_in, logfile, f'{log_statement:s}  ,  nois_std = {np.sqrt(nois_pow):.2e}, sig_max = {sig_max:.2e}')
            self.tk.print_log(params_in, logfile, f'{log_statement:s}  ,  snr_P = {snr_pow:.2f}, snr_A = {snr_amp:.2f}')
            ###
            ...
            if sig_pow <=  params_in['min_snr_P'] * nois_pow or snr_amp <=  params_in['min_snr_A']:
                ...
                ###
                self.tk.print_log(params_in, logfile, f'{log_statement:s}  ,  **poor quality: skip -FAILED- ]')
                ###

            # Save to input_dict: id_sig,id_noise
            self.write_to_dict(input_dict,['id_sig','id_noise'],[id_sig,id_noise])
        ...
        ** param == return **
        ...
        if fail:
            ...
            if 'obs' in input_dict['event']:
                ...
                tlim, id_sig, id_noise = self.read_from_dict(input_dict,['tlim', 'id_sig', 'id_noise'])
                ...
                ###
                self.tk.print_log(params_in, logfile, f'{log_statement:s}  ,  e_noi_m = {e_noi_m:.2e}, e_noi_sd = {e_noi_sd:.2e}')
            ...
            if 'syn' in input_dict['event']:
                ...
                tlim, id_sig = self.read_from_dict(input_dict,['tlim', 'id_sig'])
            ...
            if bool_1 and bool_2:  # a hard threshold
                if np.any(id_low):
                    ...
                    ###
                    self.tk.print_log(params_in, logfile, f'{log_statement:s}  ,  **reject {np.sum(id_low)} / {len(id)} low peaks')
                    ###
            ...
            if params_in['sig_win_type'] ==  0:  # use full trace
                ...
                if np.sum(id_in) < len(id):
                    ...
                    ###
                    self.tk.print_log(params_in, logfile, f'{log_statement:s}  ,  **reject {len(id) - np.sum(id_in)} / {len(id)} outside tlim [{tlim[0]:.2f},{tlim[1]:.2f}] s')
                    ###
            ...
            if not id_pks.any():
                ...
                ###
                self.tk.print_log(params_in, logfile, f'{log_statement:s}  ,  **no prominent peaks [skip]')
                ###
            ...
            i1,i2 = self.tw_det(e,t,id_pks,walkaway)
            ...
            if not params_in['filttwin']:
                ...
                ###
                self.tk.print_log(params_in, logfile, f'{log_statement:s}  ,  --- time windows --- \n' + tw_info + '---')
                self.tk.print_log(params_in, logfile, f'{log_statement:s}  ,  Save all {len(i1)} windows to {outfile:s}')
                ###
            ...
            self.write_to_dict(input_dict,['t','dt','e_noi_m','e_sig_m','min_win_span', 'nslc', 'stel', 'stla', 'stlo', 'mxf_win', 'e1', 'e', 'i1', 'id_pks', 'i2', 'twin_span'],[t,dt,e_noi_m,e_sig_m,min_win_span,nslc,stel,stla,stlo,mxf_win, e1, e, i1, id_pks, i2,twin_span])
        ...
        ** param == return **
        ...
        if fail:
            ...
            if params_in['filttwin']:
                ...
                t,e_noi_m,nslc,stel,stla,stlo,e1, i1, id_pks, i2,twin_span = self.read_from_dict(input_dict,['t','e_noi_m', 'nslc', 'stel', 'stla', 'stlo', 'e1', 'i1', 'id_pks', 'i2', 'twin_span'])
                ...
                if wsz_lim is not None:
                    ...
                    if np.any(id_sz):
                        ###
                        self.tk.print_log(params_in, logfile, f'{log_statement:s}  ,  **reject {np.sum(id_sz)} / {len(id_pks)} windows with span outside [{wsz_lim[0]}, {wsz_lim[1]}]s')
                        ###

                    i1, id_pks, i2, twin_span = self.filter_arrays(i1, id_pks, i2, twin_span, id_sz)
                ...
                if bnd2pk_t is not None:
                    ...
                    if np.any(id_sz):
                        ###
                        self.tk.print_log(params_in, logfile, f'{log_statement:s}  ,  **reject {np.sum(id_sz)} / {len(id_pks)} windows with edge-to-peak outside [{bnd2pk_t[0]}, {bnd2pk_t[1]}]s')
                        ###
                ...
                if min_amp is not None and min_amp > 0:
                    ...
                    if np.any(id_lowamp):
                        ###
                        self.tk.print_log(params_in, logfile, f'{log_statement:s}  ,  **reject {np.sum(id_lowamp)} / {len(id_pks)} windows of amp < {min_amp}')
                        ###

                    i1, id_pks, i2, twin_span = self.filter_arrays(i1, id_pks, i2, twin_span, id_lowamp)
                ...
                if min_snr is not None and min_snr > 0:
                    ...
                    if np.any(id_lowsnr):
                        ###
                        self.tk.print_log(params_in, logfile, f'{log_statement:s}  ,  **reject {np.sum(id_lowsnr)} / {len(id_pks)} windows of snr < {min_snr:.2f}')
                        ###

                    i1, id_pks, i2, twin_span = self.filter_arrays(i1, id_pks, i2, twin_span, id_lowsnr)
                ...
                if bnd2pk_A is not None:                
                    ...
                    if np.any(id_hb2p):
                        ###
                        self.tk.print_log(params_in, logfile, f'{log_statement:s}  ,  **reject {np.sum(id_hb2p)} / {len(id_pks)} windows with high edges')
                        ###

                    i1, id_pks, i2, twin_span = self.filter_arrays(i1, id_pks, i2, twin_span, id_hb2p)

                if not np.any(twin_span):
                    fail = 1
                    ###
                    self.tk.print_log(params_in, logfile, f'{log_statement:s}  ,  **no prominent twin remains....\n')
                    ###
                    ...
                    for ii in range(len(twin_span)):
                        # if twin_span[ii] < min_win_span:
                            # ###
                            # self.tk.print_log(params_in, logfile, f'{log_statement:s}  ,  **reject narrow window: {t[i1[ii]]} -- {t[i2[ii]]} = {twin_span[ii]:.1f}')
                            # ###
                        tw_info  +=  params_in['twin_outfmt'].format(nslc, stla, stlo, stel, t[i1[ii]], t[id_pks[ii]], t[i2[ii]], e1[i1[ii]], e1[id_pks[ii]], e1[i2[ii]], e_noi_m) + '\n' # need e_sig_m as final parameter when using synth data.
                    ...
                    ###
                    self.tk.print_log(params_in, logfile, f'{log_statement:s}  ,  --- time windows --- \n' + tw_info + '---\n')
                    self.tk.print_log(params_in, logfile, f'{log_statement:s}  ,  Save filtered {len(i1)} windows to {outfile}')
                    ###
                    ...
                    # Save to input_dict: 
                    self.write_to_dict(input_dict,['i1', 'id_pks', 'i2', 'twin_span'],[i1, id_pks, i2,twin_span])
        ...
        ** param == return **
        ...
        if fail:
            ...
            if params_in['twin_plot_pic']:
                ...
                e_noi_m,e_sig_m,nslc, e1, e, i1, id_pks, i2,ttt,t,tlim = self.read_from_dict(input_dict,['e_noi_m','e_sig_m', 'nslc', 'e1', 'e', 'i1', 'id_pks', 'i2','ttt','t','tlim'])
                dt = t[1] - t[0]
                twin_span = (i2 - i1) * dt
                ...
                ###
                self.tk.print_log(params_in, logfile, f'{log_statement:s}  ,  -NO PLOT REQUESTED- ')
                ###
        return input_dict, filename, seis, logfile, outfile, fail
    ...
    def tw_det(self, e, t, ipeak, walkaway, balance = False):
        ...
        for ii in range(len(ipeak)): #len(ipeak)
            ...
            # Left side
            sid, x = self.tk.subvec(t, t, [t[ipk] + dt - walkaway, t[ipk] - dt])
            dis = self.point_line_distance([x, y[sid]], [t[ipk], y[ipk]], [t[ipk] - walkaway, 0])
            mid = np.argmax(dis)  # find max dis to the dip line
            ileft[ii] = np.where(sid)[0][mid]  # left bound at t(i1)
            # Right side
            sid, x = self.tk.subvec(t, t, [t[ipk] + dt, t[ipk] + walkaway - dt])
            dis = self.point_line_distance([x, y[sid]], [t[ipk], y[ipk]], [t[ipk] + walkaway, 0])
...
class FindTwinObs(FindTwin):
    ...
    def process_one_event(self, input_dict):
        ...
        if num_files ==  0:
            # No files found...
            ###
            self.tk.print_log(params_in, logfile, f'----------////   NO-FILES-IN: {str(event)}    ////----------')
            ###

        else:
            ###
            self.tk.print_log(params_in, logfile, f'----------////    WORKING ON: {str(event)}    ////----------')
            ###

            # Loop through file list and apply functions to each
            for k, file in enumerate(files):
                log_statement = f'{self.tk.get_log_statement(event_id, file)}, [{str(k + 1)}/{str(num_files)}]'
                ...
                if len(functions) ==  0: 
                    ###
                    self.tk.print_log(params_in, logfile, f'{log_statement:s}  ,  NO_FUNCTIONS_TO_APPLY.....')
                    ###
