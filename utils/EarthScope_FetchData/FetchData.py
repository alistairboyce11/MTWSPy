...
# \\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\ # 
def get_event_metadata(data_loc, year):
    '''
    Use Iris Event server to find all events for given year using FetchEvent perl script
    Try to read this into pandas df
...
def get_datacenters():
    '''
    Function to produce dataframe of datacenter names and their locations online.
...
# \\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\ # 
def get_station_metadata(data_loc, year, channel, dc_name, dc_loc_string):
    '''
    Use Given datacenter server to find all available stations for given year using FetchMetadata perl script
    Try to read this into pandas df
...
# \\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\ # 
def split_dataframe_by_ev_id(df, packet_request_size):
    '''
...
# \\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\ # 
def write_chunk_file(data_loc, year, output_directory, ev_id_count, chunk):
    '''
...
# \\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\ # 
def process_one_chunk(input_dict):
    '''
...
# \\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\ # 
def get_data(data_loc, year, channel, main_function, function_list, packet_request_size, do_parallel, jobs, processing_channel):
    '''
    ...
    # Get events
    cmt_table = get_event_metadata(data_loc, year)
    ...
    for i, dc_row in datacenters.iterrows():
        ...
        out_stat_df, inv_loc = get_station_metadata(data_loc, year, channel, dc_name, dc_loc_string)
        ...
        chunks, ev_ids, ev_id_counts = split_dataframe_by_ev_id(request_df, packet_request_size)
        ...
        if len(chunks) == len(ev_ids) and len(ev_ids) == len(ev_id_counts):
            ...
            for i in range(len(chunks)):
                ...
                chunk_f_name = write_chunk_file(data_loc, year, output_directory, ev_id_count, chunk)
...
# \\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\ # 
def update_headers_seis(input_dict, seis, fail):
    '''
...
# \\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\ # 
def rem_inst_resp(input_dict, seis, fail):
    '''
...
# \\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\ # 
def remove_trend_taper(input_dict, seis, fail):
    '''
...
# \\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\ # 
def filter_seis(input_dict, seis, fail):
    '''
    Filter seismogram
...
# \\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\ # 
def interpolate_seis(input_dict, seis, fail):
    '''
...
# \\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\ # 
def save_seis(input_dict, seis, fail):
    '''
...
# \\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\ # 
def get_input_dicts_rotate(year, channel, data_loc, function_list, do_parallel, jobs, processing_channel):
    '''
...
# \\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\ # 
def apply_serial(data_loc, year, channel, main_function, function_list, do_parallel, jobs, processing_channel):
    '''
    ...
    input_dicts = get_input_dicts_rotate(year, channel, data_loc, function_list, do_parallel, jobs, processing_channel)
...
# \\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\ # 
def apply_parallel(data_loc, year, channel, main_function, function_list, do_parallel, jobs, processing_channel):
    '''
...
# \\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\ # 
def execute(data_loc, year, channel, main_function, function_list, do_parallel, jobs, processing_channel):
    '''
    Takes main function inputs and passes to serial or parallel executer, see below.
    '''
    if do_parallel:
        # Execute in parallel
        apply_parallel(data_loc, year, channel, main_function, function_list, do_parallel, jobs, processing_channel)
    else:
        # Execute in series
        apply_serial(data_loc, year, channel, main_function, function_list, do_parallel, jobs, processing_channel)
    return
...
def check_length(input_dict, file_vertical, file_east, file_north, vert_component, east_component, north_component, fail):
    '''
    Check trace length - Often traces are one sample longer than the others - test this trimming.
...
def main():
    ...
    if len(sys.argv) != 3:
        ...
        print("USAGE:   python FetchData.py <year> <channel>")
        print("EXAMPLE: python FetchData.py 2008 LH")
        sys.exit()
    else:
        year = int(sys.argv[1])
    ...
    # Find earthquakes, stations, make most appropriate request files. (Use FetchEvent, FetchMetaData)
    # Request using FetchData, process directly in obspy e.g., remove inst. resp save to py_formatted.
    main_function = [process_one_chunk]

    function_list = [rem_inst_resp, update_headers_seis, remove_trend_taper, filter_seis, interpolate_seis, save_seis]

    get_data(data_loc, year, channel, main_function, function_list, packet_request_size, do_parallel, jobs, processing_channel)
    ...
    execute(data_loc, year, channel, main_function, function_list, do_parallel, 96, processing_channel)
