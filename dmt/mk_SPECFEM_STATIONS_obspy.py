...
def get_inventory(home, year, network, station, location, channel):

    starttime = UTCDateTime(str(year)+"-01-01")
    endtime = UTCDateTime(str(year)+"-12-31")
    clients=['AUSPASS','ETH','GEOFON','GEONET','ICGC','INGV','IPGP','IRIS','KNMI','KOERI','LMU','NCEDC','NIEP','NOA','ORFEUS','RESIF','SCEDC','UIB-NORSAR','USP']
    # clients=['AUSPASS','ETH','GEOFON']
...
def main():
    ...
    for year in listyears:
        inv=get_inventory(home, year, network, station, location, channel)
